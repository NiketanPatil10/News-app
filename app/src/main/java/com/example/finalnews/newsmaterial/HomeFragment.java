...
public class HomeFragment extends Fragment {
    ...
    void getNews(){
        ApiUtilities.getApiInterface().getTopHeadlines("dfRXTGJ3LHJaSpw2mrE7wydaxXoyPfDYUPi1cqT1","in","general").enqueue(new Callback<Root>() {
            @Override
            public void onResponse(Call<Root> call, Response<Root> response) {
                if (response.isSuccessful()){
                    ...
                    datumArrayList.addAll(response.body().getData());
                    datumArrayList.addAll(response.body().getData());
                    Collections.shuffle(datumArrayList);
